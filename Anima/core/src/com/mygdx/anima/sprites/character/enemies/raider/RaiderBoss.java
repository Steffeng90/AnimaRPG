...
public class RaiderBoss extends EnemyHumanoid {
    ...
    public void coordinateWalking(HumanoideSprites zielSprite, float dt) {
        ...
        if (einheitsvector.y > 0 && einheitsvector.y > Math.abs(einheitsvector.x)) {
            setCurrentRichtung(2);
        } else if (einheitsvector.x > 0 && einheitsvector.x > Math.abs(einheitsvector.y)) {
            setCurrentRichtung(1);
        } else if (einheitsvector.y < 0 && Math.abs(einheitsvector.y) > Math.abs(einheitsvector.x)) {
    ...
    public void walkingVelo(HumanoideSprites hero, Vector2 v2) {
        ...
        if ((getb2bodyX() - hero.getb2bodyX()) < 0.1f && (getb2bodyX() - hero.getb2bodyX()) > -0.1f) {
            velocity.x = 0;
        } else if ((velocity.x > -(float) getGeschwindigkeitLaufen() / 10 && velocity.x < (float) getGeschwindigkeitLaufen() / 10) || (velocity.x >= getGeschwindigkeitLaufen() / 10 && v2.x < 0) || (velocity.x <= -getGeschwindigkeitLaufen() / 10 && v2.x > 0))
            velocity.x = v2.x;
    ...
    public void attack() {
        ...
        switch (getCurrentRichtung()) {
            ...
            case Rechts:
                richtungsVector = new Vector2(20 / AnimaRPG.PPM, -8 / AnimaRPG.PPM);
                break;
            case Links:
                richtungsVector = new Vector2(-20 / AnimaRPG.PPM, -8 / AnimaRPG.PPM);
                break;
            case Oben:
                richtungsVector = new Vector2(0, 12 / AnimaRPG.PPM);
                break;
            case Unten:
                richtungsVector = new Vector2(0, -28 / AnimaRPG.PPM);
                break;
            default:
        ...
        meleeFixtureDefinieren(richtungsVector,25);
    }
}
