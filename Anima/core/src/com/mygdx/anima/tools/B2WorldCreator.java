...
public class B2WorldCreator {
    ...
    public B2WorldCreator(Playscreen screen) {
        ...
        if (map.getLayers().get("barrieren") != null)
        {
            ...
            for (MapObject object : map.getLayers().get("barrieren").getObjects().getByType(CircleMapObject.class))
            {
                Gdx.app.log("Circlefound","");
                Circle circle = ((CircleMapObject) object).getCircle();
                bdef.type = BodyDef.BodyType.StaticBody;
                bdef.position.set(circle.x/ AnimaRPG.PPM,circle.y/ AnimaRPG.PPM);
                body = world.createBody(bdef);
                cshape.setRadius(circle.radius/ AnimaRPG.PPM);
                cshape.setPosition(new Vector2(circle.x/ AnimaRPG.PPM,circle.y/ AnimaRPG.PPM));
                //set ((cic.getWidth() / 2) / AnimaRPG.PPM, (rect.getHeight() / 2) / AnimaRPG.PPM);
                fdef.shape = cshape;
                fdef.filter.categoryBits = BARRIERE_BIT;
                fdef.filter.maskBits = HERO_BIT | ENEMY_BIT | OBJECT_BIT | ARROW_BIT;
                body.createFixture(fdef);
